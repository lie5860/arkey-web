...
node "$ROOT/scripts/generate-firmware-contract.mjs" --check
...
if [ "$ACTUAL_QMK_COMMIT" != "$EXPECTED_QMK_COMMIT" ] && [ "${ARKEY_ALLOW_UNTESTED_QMK:-0}" != "1" ]; then
  ...
  echo "Set ARKEY_ALLOW_UNTESTED_QMK=1 only after reviewing and testing the new source." >&2
  exit 1
fi
...
cleanup() {
  ...
  rm -f "$TARGET/arkey.c" "$TARGET/arkey.h" "$TARGET/arkey_generated.h" "$TARGET/rgb_matrix_kb.inc"
}
trap cleanup EXIT INT TERM

cp "$ROOT/firmware/qmk/arkey.c" "$TARGET/arkey.c"
cp "$ROOT/firmware/qmk/arkey.h" "$TARGET/arkey.h"
cp "$ROOT/firmware/qmk/arkey_generated.h" "$TARGET/arkey_generated.h"
...
mkdir -p "$ROOT/build"
cp "$QMK_HOME/keychron_q6_pro_ansi_encoder_via.bin" "$ROOT/build/arkey-q6-pro-ansi-v0.1.0.bin"
if command -v dfu-suffix >/dev/null 2>&1; then
  dfu-suffix -c "$ROOT/build/arkey-q6-pro-ansi-v0.1.0.bin"
else
  echo "Warning: dfu-suffix is unavailable; the compiler succeeded but the DFU suffix was not independently checked." >&2
fi
echo "Built: $ROOT/build/arkey-q6-pro-ansi-v0.1.0.bin"
